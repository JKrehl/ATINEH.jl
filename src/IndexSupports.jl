...
convert{T<:Tuple{IndexSupportPoint}}(::Type{T}, ::Number) = ()

struct IndexReturningArray{N, V} <: AbstractArray{Tuple{IndexSupportPoint{N, V}},N}
    _size::NTuple{N, Int}
...
struct FlatIndexReturningArray{N, V} <: AbstractArray{Tuple{IndexSupportPoint{N, V}},N}
    _size::NTuple{N, Int}
